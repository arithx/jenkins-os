...
# via parameters).
timeout --signal=SIGQUIT 300m bin/cork enter --experimental -- region=$region bucket=$bucket object=$object compartment=$compartment NAME=$NAME OCI_SHAPE=$OCI_SHAPE sh -ex << 'EOF'
pyvenv ocienv && ocienv/bin/pip install oci-cli --upgrade;
ln -s /mnt/host/source/.oci ~/.oci
#sudo chown -R jenkins:jenkins ~/.oci
...
trap 'ocienv/bin/oci compute image delete --image-id "${image_id}" --force && rm -rf ~/.oci/' EXIT
