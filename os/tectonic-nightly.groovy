            ...
            withDockerContainer(tectonic_smoke_test_env_image) {
              unstash 'installer'
                sh '''#!/bin/bash -ex
                  ...
                  # Delete aws key-pair
                  trap ssh-add -d $sshdir/$TF_VAR_tectonic_aws_ssh_key
                  trap aws ec2 delete-key-pair --key-name $TF_VAR_tectonic_aws_ssh_key --region us-west-2
                  trap rm $sshdir/$TF_VAR_tectonic_aws_ssh_key
                  trap rm $sshdir/$TF_VAR_tectonic_aws_ssh_key.pub
                '''
            }
          }
        }
